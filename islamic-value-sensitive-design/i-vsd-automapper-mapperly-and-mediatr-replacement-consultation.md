...
- `ICommand`, `ICommand<TResult>`, `ICommandHandler<TCommand>`, `ICommandHandler<TCommand, TResult>` for state changes. The repository's commands overwhelmingly return values (only 6 of ~331 command handlers return `Unit`), so a result-bearing command interface is required; strict void-only CQS would force artificial follow-up queries.
...
7. **Retire `Unit`.** The six `IRequest<Unit>` usages become non-generic `ICommand`.
...
| E08 | same grep wave | MediatR: 910 `IRequestHandler`, 848 request declarations, 947 `Send`/`Publish`, 1,406 files in Application, 181 in API, 3 in Infrastructure; 2 notifications, 3 notification handlers; 0 streams/pre-post-processors/exception handlers; 6 `Unit` usages; 112 test files, 137 substitutes. |
