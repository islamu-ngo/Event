...
public sealed class StudioController(
    IQueryHandler<GetStudioContextQuery, StudioContextDto> getContextQueryHandler,
    IResourceAssembler<StudioContextDto, StudioContextDto> assembler) : ControllerBase
{
    ...
    {
        StudioContextDto context = await getContextQueryHandler.QueryAsync(new GetStudioContextQuery(actorId), cancellationToken);
