...
public sealed class NativeRegistrationModeHttpTests
{
    [Test]
    public async Task AnonymousReads_PreserveValuesAndMissingDetailBehavior()
    {
        ...
        using (var scope = factory.Services.CreateScope())
        {
            ...
            db.RegistrationModes.AddRange(
                new RegistrationMode { Id = 8008, MasterCode = "TEST_FIRST", FullName = "Test First", Description = "First lookup item" },
                new RegistrationMode { Id = 8002, MasterCode = "TEST_SECOND", FullName = "Test Second" });
            await db.SaveChangesAsync();
        }

        using var listResponse = await client.GetAsync("/api/RegistrationMode");
        await Assert.That(listResponse.StatusCode).IsEqualTo(HttpStatusCode.OK);
        var list = await listResponse.Content.ReadFromJsonAsync<List<RegistrationModeListDto>>()
            ?? throw new InvalidOperationException("Expected a registration-mode array.");
        await Assert.That(list.Single(item => item.Id == 8008)).IsEqualTo(new RegistrationModeListDto
        {
            Id = 8008, MasterCode = "TEST_FIRST", FullName = "Test First", Description = "First lookup item"
        });
        await Assert.That(list.Single(item => item.Id == 8002)).IsEqualTo(new RegistrationModeListDto
        {
            Id = 8002, MasterCode = "TEST_SECOND", FullName = "Test Second"
        });

        using var detailResponse = await client.GetAsync("/api/RegistrationMode/8002");
        await Assert.That(detailResponse.StatusCode).IsEqualTo(HttpStatusCode.OK);
        await Assert.That(await detailResponse.Content.ReadFromJsonAsync<RegistrationModeDto>()).IsEqualTo(new RegistrationModeDto
        {
            Id = 8002, MasterCode = "TEST_SECOND", FullName = "Test Second"
        });
        ...
        using var missing = await client.GetAsync("/api/RegistrationMode/2147483647");
    ...
    public async Task EmptyCatalog_RemainsAnEmptyJsonArray()
    {
        ...
        {
            var db = scope.ServiceProvider.GetRequiredService<ExploreDbContext>();
            db.RegistrationModes.RemoveRange(db.RegistrationModes);
            await db.SaveChangesAsync();
        }
        using var response = await client.GetAsync("/api/RegistrationMode");
    ...
    public async Task PublicContract_PreservesOperationIdsTagsAndCachePolicies()
    {
        ...
        [
            ("/api/registrationmode", "GetRegistrationModes", nameof(RegistrationModeController.GetAll), "LookupData"),
            ("/api/registrationmode/{id}", "GetRegistrationModeById", nameof(RegistrationModeController.GetById), "DetailData")
        ];
        foreach (var contract in expected)
        {
            ...
            await Assert.That(operation.GetProperty("tags")[0].GetString()).IsEqualTo("RegistrationMode");
            await Assert.That(operation.GetProperty("x-endpoint-class").GetString()).IsEqualTo("Public");
            var method = typeof(RegistrationModeController).GetMethod(contract.Method)!;
        ...
        var detailOperation = paths.GetProperty("/api/registrationmode/{id}").GetProperty("get");
