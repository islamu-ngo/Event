...
## Categorized Note Verification

- [ ] Review categorized primary entries: breaking status takes precedence over type; Features, Bug Fixes, Performance, and Other Improvements follow, with empty groups omitted. Each primary entry appears once; impact and complete-range references remain intact.
- [ ] Review applicable breaking, migration, configuration, security, API, and operator evidence independently of category headings.
- [ ] Promote the engine and packaged formatter template together. Do not mutate an accepted bundle or rewrite signed historical notes. If generated bytes differ before `B`, follow [generated-note mismatch recovery](RELEASE_RUNBOOK.md#generated-note-mismatch-recovery).
