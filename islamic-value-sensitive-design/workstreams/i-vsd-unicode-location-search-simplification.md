...
PR38 and earlier Unicode commits are now published in origin/develop. Its newly merged legacy tests were reconciled without compatibility code: two redundant files removed, two composition examples retained in existing Domain tests. Final local Domain1136/1136, normalization27/27, Release0errors, Architecture578passed/1existing skip and PostgreSQL Unicode queries8/8 pass. Independent review confirms the unchanged production/migration/structured-corpus inputs retain their five-engine proof.

Separate [secret-binding uniqueness](../dev/backlog/secret-binding-provider-uniqueness.md), [ATProto fixture IDs](../dev/backlog/atproto-snapshot-fixture-record-ids.md) and [CI tool prerequisites](../dev/backlog/ci-verification-tool-prerequisites.md) are quarantined under PR38's governance, not silently passed or absorbed into Unicode scope. The fifth structured skip still has3provider passes/2failures. Both immutable Unicode fragments received schema/impact/link review and task commit ranges pass native preflight; no full release preparation was performed. Final local test/documentation commits still need publication authority and implementation-head CI follow-through. These status corrections preserve the reviewed Unicode design and IVSD mappings, without certifying whole-pipeline health, arbitrary retained-data upgrades, future ICU profiles, stakeholder outcomes or scholarly review.
