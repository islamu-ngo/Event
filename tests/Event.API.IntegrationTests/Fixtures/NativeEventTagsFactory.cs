...
public sealed class NativeEventTagsFactory : AuthenticatedWebApplicationFactory
{
    private readonly string? _databasePath;

    public NativeEventTagsFactory(bool relational = false)
    {
        if (relational)
            _databasePath = Path.Combine(Path.GetTempPath(), $"native-event-tags-{Guid.CreateVersion7():N}.db");
