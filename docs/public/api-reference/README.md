...
ISLAMU Event exposes a versioned HAL/REST API backed by explicitly authorized application operations. The draft HTTP API version is `0.1` (pre-v1 specification): breaking changes may occur during active pre-release development as endpoint contracts are refined. Pin generated contracts and review the API changelog before upgrading.
