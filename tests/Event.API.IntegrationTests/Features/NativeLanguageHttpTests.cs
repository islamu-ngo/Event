...
public sealed class NativeLanguageHttpTests
{
    [Test]
    public async Task AnonymousReads_PreserveValuesAndMissingDetailBehavior()
    {
        ...
        using (var scope = factory.Services.CreateScope())
        {
            ...
            db.Languages.AddRange(
                new Language { Id = 8008, MasterCode = "TEST_FIRST", FullName = "Test First", Description = "First lookup item" },
                new Language { Id = 8002, MasterCode = "TEST_SECOND", FullName = "Test Second" });
            await db.SaveChangesAsync();
        }

        using var listResponse = await client.GetAsync("/api/Language");
        await Assert.That(listResponse.StatusCode).IsEqualTo(HttpStatusCode.OK);
        var list = await listResponse.Content.ReadFromJsonAsync<List<LanguageListDto>>()
        ...
        using var missing = await client.GetAsync("/api/Language/2147483647");
    ...
    public async Task EmptyCatalog_RemainsAnEmptyJsonArray()
    {
        ...
        using var response = await client.GetAsync("/api/Language");
    ...
    public async Task PublicContract_PreservesOperationIdsTagsAndCachePolicies()
    {
        ...
        [
            ("/api/language", "GetLanguages", nameof(LanguageController.GetAll), "LookupData"),
            ("/api/language/{id}", "GetLanguageById", nameof(LanguageController.GetById), "DetailData")
        ];
        foreach (var contract in expected)
        {
            ...
            await Assert.That(operation.GetProperty("tags")[0].GetString()).IsEqualTo("Language");
            await Assert.That(operation.GetProperty("x-endpoint-class").GetString()).IsEqualTo("Public");
            var method = typeof(LanguageController).GetMethod(contract.Method)!;
        ...
        var detailOperation = paths.GetProperty("/api/language/{id}").GetProperty("get");
