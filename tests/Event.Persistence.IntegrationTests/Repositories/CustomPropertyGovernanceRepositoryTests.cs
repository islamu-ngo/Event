...
public sealed class CustomPropertyGovernanceSqliteRepositoryTests
{
    ...
    public async Task ReportUsesPersistedRowsAndFiltersBeforeCountingAndPaging()
    {
        ...
        try
        {
            ...
            {
                Role = PrimaryDatabaseRole.Migrator, Provider = PrimaryDatabaseProvider.Sqlite, Database = path
            });
...
internal static class CustomPropertyGovernanceRepositoryContract
{
    ...
    private static async Task<Guid> SeedAsync(ExploreDbContext db)
    {
        ...
        {
            Id = Guid.CreateVersion7(), FullName = "Governance matrix", Slug = $"governance-{Guid.CreateVersion7():N}",
            TenantStatusId = 2, TenantStatus = null!
        };
        var user = new Explore.Domain.User
        {
            Id = Guid.CreateVersion7(), Pii = new Explore.Domain.UserPii
            {
                Email = $"governance-{Guid.CreateVersion7():N}@example.test", FirstName = "Governance", LastName = "Matrix"
            }
        };
        ...
        {
            Id = Guid.CreateVersion7(), ActorTypeId = 1, ActorType = null!, UserId = user.Id,
        ...
        {
            Id = Guid.CreateVersion7(), Title = "Governance matrix event", TenantId = tenant.Id, Tenant = tenant,
            ActorId = actor.Id, Actor = actor, EventStatus = null!, EventFormatId = 1, EventFormat = null!,
        ...
        {
            Id = Guid.CreateVersion7(), TenantId = tenant.Id, Tenant = tenant, EventId = eventEntity.Id, Event = eventEntity,
            Title = "Governance session", EventSessionKindId = 1, RegistrationModeId = 1, ConcurrencyStamp = Guid.CreateVersion7()
        };
        ...
        {
            Id = Guid.CreateVersion7(), TenantId = tenant.Id, EventSessionId = session.Id,
            Namespace = "tenant.custom", Key = "d-layer2", DisplayName = "Session analytics",
            IsActive = true, IsAnalyticsRelevant = true, PropertyType = Explore.Domain.Enums.PropertyType.Text,
            ExposureLevel = Explore.Domain.Enums.ExposureLevel.TenantAdminOnly, ConcurrencyStamp = Guid.CreateVersion7()
        });
        db.EventCustomPropertyValues.AddRange(new Explore.Domain.EventCustomPropertyValue
        {
            Id = Guid.CreateVersion7(), TenantId = tenant.Id, EventId = eventEntity.Id,
            EventCustomPropertyDefinitionId = layer1.Id, TextValue = "not-report-content",
            UpdatedAt = new DateTime(2026, 8, 1, 12, 0, 0, DateTimeKind.Utc), ConcurrencyStamp = Guid.CreateVersion7()
        }, new Explore.Domain.EventCustomPropertyValue
        {
            Id = Guid.CreateVersion7(), TenantId = tenant.Id, EventId = eventEntity.Id,
            EventCustomPropertyDefinitionId = layer1.Id, Ordinal = 1, TextValue = "deleted-not-report-content",
            IsDeleted = true, UpdatedAt = new DateTime(2026, 8, 2, 12, 0, 0, DateTimeKind.Utc), ConcurrencyStamp = Guid.CreateVersion7()
        });
        await db.SaveChangesAsync();
        return tenant.Id;
        ...
        Explore.Domain.EventCustomPropertyDefinition Definition(string key) => new()
        {
            ...
            PropertyType = Explore.Domain.Enums.PropertyType.Text, ExposureLevel = Explore.Domain.Enums.ExposureLevel.Public,
