...
SingleTenant private management permits the existing control-plane detail and activation routes only for the fixed default tenant. The controller rejects arbitrary targets; the existing instance-setting authorization still decides who can read or activate. Machine principals, ordinary members and wrong-tenant administrators receive no new authority. Tenant identity/branding edits retain their separate exact tenant-admin authorization: the completed setup administrator holds both persisted platform and default-tenant grants. HAL is server-authored; the SingleTenant detail removes fleet-only links rather than exposing actions the mode forbids. No role/claim gating is added to Blazor.
