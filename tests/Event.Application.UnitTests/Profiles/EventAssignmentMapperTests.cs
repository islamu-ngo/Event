...
public sealed class EventAssignmentMapperTests
{
    ...
    public async Task CreateAssignments_UseTrustedTenantAndOnlyValidatedRelationshipIds()
    {
        ...
        var tagResult = await new CreateEventTagsCommandHandler(tagAssignments, events, tags, tenant).ExecuteAsync(new CreateEventTagsCommand { EventTagsDto = new CreateEventTagsDto { EventId = EventId, TagId = ClassificationId, TenantId = ClassificationId } }, CancellationToken.None);
