...
public sealed class NativeCategoryTypeOperationTests
{
    [Test]
    public async Task CategoryTypeCapability_RegistersTwoScopedQueryPorts()
    {
        var services = new ServiceCollection();
        services.AddNativeOperations(
        [
            typeof(GetCategoryTypeDetailsRequest), typeof(GetCategoryTypeDetailsRequestHandler),
            typeof(GetCategoryTypeListRequest), typeof(GetCategoryTypeListRequestHandler)
        ]);
        ...
        await Assert.That(ports.Select(port => port.ServiceType.GetGenericArguments()[0]))
            .IsEquivalentTo(new[] { typeof(GetCategoryTypeDetailsRequest), typeof(GetCategoryTypeListRequest) });
    }

    [Test]
    public async Task MissingDetail_DeclaresNullableHandlerResult()
    {
        var method = typeof(GetCategoryTypeDetailsRequestHandler)
            .GetMethods(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly)
            .Single(method => method.ReturnType == typeof(Task<CategoryTypeDto>));
