...
5. **Self-Documenting Code & Natural Documentation Headers**: Source code (`.cs`, `.razor`, `.css`, project files) must be self-documenting through Clean Architecture naming, folder structure, and standard C# XML doc comments (`/// <summary>`) where non-inferable. The legacy `ABOUTME:` comment prefix is officially retired for source code (agents must not generate it). Documentation files in `docs/internal/` and `.agents/` use natural Markdown metadata blocks or standard frontmatter rather than synthetic code comments. See [`docs/internal/GOVERNANCE.md`](docs/internal/GOVERNANCE.md#header--file-metadata-policy-natural-metadata--code-retirement).
