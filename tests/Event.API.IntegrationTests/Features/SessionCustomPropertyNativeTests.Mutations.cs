...
public sealed partial class SessionCustomPropertyNativeTests
{
    ...
    [Arguments(false)]
    [Arguments(true)]
    public async Task EmptyOwnedSession_CreateInvalidatesEveryWarmPage_OnlyAfterCommit(bool failCommit)
    {
        ...
        using (var seedScope = Scope(factory, PlatformDefaults.DefaultTenantId))
        {
            ...
            var source = await db.EventSessions.Include(row => row.Event).SingleAsync(row => row.Id == data.SessionId);
            var emptyEvent = await EventScenarioSeed.SeedPublishedEventAsync(db, source.Event.ActorId, PlatformDefaults.DefaultTenantId);
            emptySessionId = await db.EventSessions.Where(row => row.EventId == emptyEvent.EventId).Select(row => row.Id).SingleAsync();
            var owned = await seedScope.ServiceProvider.GetRequiredService<IEventSessionRepository>().GetById(emptySessionId);
        ...
        foreach (var (page, size) in variants)
        {
            ...
            var empty = await ListAsync(factory, PlatformDefaults.DefaultTenantId, emptySessionId, page, size);
            ...
            await ListAsync(factory, data.ForeignTenantId, data.ForeignSessionId, page, size);
        ...
        foreach (var (page, size) in variants)
        {
            var emptyHit = await ListAsync(factory, PlatformDefaults.DefaultTenantId, emptySessionId, page, size);
        ...
        try
        {
            response = await client.PostAsJsonAsync(Root, CreateDto(emptySessionId));
        }
        finally
        {
        ...
        using (response)
        {
            ...
            {
                var before = factory.Reads.Count(PlatformDefaults.DefaultTenantId);
                var current = await ListAsync(factory, PlatformDefaults.DefaultTenantId, emptySessionId, page, size);
                ...
                var foreign = await ListAsync(factory, data.ForeignTenantId, data.ForeignSessionId, page, size);
            ...
            var persisted = await verifyScope.ServiceProvider.GetRequiredService<IEventSessionCustomPropertyRepository>()
                .GetDefinitionsForSessionPaged(emptySessionId, 1, 20);
    ...
    public async Task PendingCommit_KeepsCommittedPagesVisibleUntilRelease()
    {
        ...
        try
        {
            ...
            await Assert.That((await ListAsync(factory, PlatformDefaults.DefaultTenantId, data.SessionId, 2, 1)).TotalCount).IsEqualTo(3);
    ...
    public async Task RejectedWritesAndForeignDeletePurge_PreserveStateAndExactErrors()
    {
        ...
        using var duplicate = await client.PostAsJsonAsync(Root, CreateDto(data.SessionId) with { Key = "own" });
        await ProblemAsync(duplicate, HttpStatusCode.BadRequest, "validation_failed", "eventSessionCustomPropertyDefinition");
    ...
    public async Task Mutation_CommitsOrRollsBack_AndOnlyThenInvalidatesAllTenantPages(string mutation, bool failCommit)
    {
        ...
        {
            await ListAsync(factory, PlatformDefaults.DefaultTenantId, data.SessionId, page, size);
            await ListAsync(factory, data.ForeignTenantId, data.ForeignSessionId, page, size);
        }
        ...
        foreach (var (page, size) in new[] { (1, 1), (2, 1), (1, 2) })
        {
            var own = await ListAsync(factory, PlatformDefaults.DefaultTenantId, data.SessionId, page, size);
        ...
        await Assert.That((await repository.GetDefinitionsForSessionPaged(data.SessionId, 1, 20)).TotalCount).IsEqualTo(total);
    ...
    public async Task PostCommitCancellation_DoesNotSkipPageInvalidation()
    {
        ...
        var result = await scope.ServiceProvider.GetRequiredService<ICommandHandler<CreateEventSessionCustomPropertyDefinitionCommand, BaseCommandResponse<Guid>>>()
            .ExecuteAsync(new() { DefinitionDto = CreateDto(data.SessionId) }, cancellation.Token);
    ...
    public async Task PurgeDependencies_BlockWithoutEvictionOrNewAudit(string blocker)
    {
        ...
        else
        {
            ...
            if (blocker == "audit") db.AuditLogs.Add(new AuditLog
            {
                ...
                EntityType = nameof(EventSessionCustomPropertyDefinition), EntityId = data.DefinitionId.ToString(),
                Action = "RetainedReference", Timestamp = DateTime.UtcNow
            });
            else (await db.EventSessionCustomPropertyDefinitions.SingleAsync(row => row.Id == data.DefinitionId)).SourceTemplateDefinitionId = Guid.CreateVersion7();
            await db.SaveChangesAsync();
        }
        await ListAsync(factory, PlatformDefaults.DefaultTenantId, data.SessionId, 2, 1);
        using var response = await MutateAsync(client, data, "purge");
        await ProblemAsync(response, HttpStatusCode.BadRequest, "validation_failed", "eventSessionCustomPropertyDefinition");
        var reads = factory.Reads.Count(PlatformDefaults.DefaultTenantId);
        await Assert.That((await ListAsync(factory, PlatformDefaults.DefaultTenantId, data.SessionId, 2, 1)).TotalCount).IsEqualTo(3);
        await Assert.That(factory.Reads.Count(PlatformDefaults.DefaultTenantId)).IsEqualTo(reads);
        using var scope = Scope(factory, PlatformDefaults.DefaultTenantId);
    ...
    {
        if (mutation == "create") return await client.PostAsJsonAsync(Root, CreateDto(data.SessionId));
        ...
        if (mutation == "purge")
        {
            ...
            {
                Content = JsonContent.Create(new PurgeCustomPropertyDefinitionDto("Disposable session definition"))
            };
            return await client.SendAsync(purge);
        }
        var patch = mutation == "update"
            ? new UpdateEventSessionCustomPropertyDefinitionDto { Metadata = new() { DisplayName = "Updated" } }
            : new UpdateEventSessionCustomPropertyDefinitionDto
            {
