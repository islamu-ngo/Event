...
Deployable Dockerfiles must use tag-plus-digest base image references, for example `mcr.microsoft.com/dotnet/aspnet:10.0@sha256:<digest>`. For multi-architecture container builds, the pinned digest must be the root OCI image index (manifest list) digest covering both `linux/amd64` and `linux/arm64`. The human-readable tag preserves maintainer intent while the digest fixes the resolved image. `Workflow Security` enforces this with `.ci/scripts/validate-dockerfile-base-images.cs` for deployable Dockerfiles, supporting standard `FROM --platform=$BUILDPLATFORM` cross-compilation syntax. Build stages use `--platform=$BUILDPLATFORM` on the host runner architecture alongside `ARG TARGETARCH` and `-a $TARGETARCH` so compilation runs natively without slow QEMU compiler emulation. Dependabot's `docker` ecosystem entries update those digests weekly through grouped `docker-base-images` PRs.
...
| Container multi-arch build / SBOM / provenance / Trivy / attestation / promotion verification | Deploy-only | No | Required before deployment workflows call Coolify; builds multi-architecture OCI indexes (`linux/amd64`, `linux/arm64`) via Buildx and QEMU (`docker/setup-qemu-action`); retained evidence includes registry manifest/index output, immutable primary-registry tag promotion evidence, vulnerability scan artifacts, attestation verification JSON, and checksum manifests. |
